...
router.delete('/deletecommodity/:id', function(req, res) {
  req.commodity.findByIdAndDelete(req.params.id, function(err, result){
    res.send((err === null) ? { msg: '' } : { msg: err });
  });
});
...
router.put('/updatecommodity/:id', function (req, res) {
  ...
  req.commodity.findByIdAndUpdate(commodityToUpdate, {status: newStatus}, function(err, result){
    res.send((err === null) ? { msg: '' } : { msg: err });
  });
